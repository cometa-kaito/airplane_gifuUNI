...
| `safe_angle <deg>` | 姿勢角しきい値（`|roll|` または `|pitch|` がこれを超えると AUTO→MANUAL に強制復帰）。既定 60、`0` で無効、上限 180 |
...
何かコマンドを 1 行受信した時点で自動的に `[FAILSAFE] cleared (uplink resumed)` を送って通常モードへ復帰する。
