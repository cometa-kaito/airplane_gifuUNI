...
ファイル：`<Arduino スケッチブック>/libraries/Seeed_Arduino_LSM6DS3/LSM6DS3.cpp`
（Windows 既定: `%USERPROFILE%\Documents\Arduino\libraries\...`、macOS 既定: `~/Documents/Arduino/libraries/...`）
108 行目付近を編集：
...
PowerShell で（`<repo-root>` は本リポジトリを clone したパスに読み替え）：

```powershell
# 演習プログラムフォルダに移動
cd "<repo-root>\制御系サンプルプログラム等\30_演習プログラム"
